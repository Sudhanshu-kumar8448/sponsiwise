...
const typeVisuals: Record<string, TypeVisual> = {
  COMPANY_CREATED: { icon: "🏢", label: "Company Created", color: "gray" },
  COMPANY_VERIFIED: { icon: "✅", label: "Company Verified", color: "green" },
  COMPANY_REJECTED: { icon: "❌", label: "Company Rejected", color: "red" },
  SPONSORSHIP_CREATED: { icon: "🤝", label: "Sponsorship Created", color: "purple" },
  ...
  NOTIFICATION_CREATED: { icon: "🔔", label: "Notification", color: "blue" },
...
function TimelineItem({ entry, isLast }: { entry: CompanyTimelineEntry; isLast: boolean }) {
  ...
  return (
    <div className="relative flex gap-4 pb-8 last:pb-0">
      ...
      {!isLast && (
        <div className={`absolute left-[17px] top-10 bottom-0 w-0.5 ${colors.line}`} />
      )}
      {/* Icon circle */}
      <span className={`relative z-10 flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-sm ${colors.iconBg}`}>{visual.icon}</span>
      {/* Content card */}
      <div className="flex-1 rounded-lg bg-white px-4 py-3 shadow-sm border border-gray-100">
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {entry.status && (
              <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium ${colors.badge} ${colors.badgeText}`}>
                <span className={`inline-block h-1.5 w-1.5 rounded-full ${colors.dot}`} />
                {entry.status}
              </span>
            )}
          </div>
          <time className="shrink-0 text-xs text-gray-400">{formatTimestamp(entry.timestamp)}</time>
        </div>
        {entry.description && <p className="mt-1 text-sm text-gray-600">{entry.description}</p>}
        {entry.recipient && (
          <p className="mt-1 text-xs text-gray-500"><span className="font-medium">To:</span> {entry.recipient}</p>
        )}
        {entry.subject && (
          <p className="mt-0.5 text-xs text-gray-500"><span className="font-medium">Subject:</span> {entry.subject}</p>
        )}
        {entry.actorId && (
          <p className="mt-1 text-xs text-gray-400">By: {entry.actorId.slice(0, 8)}…{entry.actorRole && (<span className="ml-1 rounded bg-gray-100 px-1.5 py-0.5 text-[10px] font-medium text-gray-500">{entry.actorRole}</span>)}</p>
        )}
      </div>
    </div>
  );
...
export default function CompanyLifecycleTimeline({ timeline }: { timeline: CompanyTimelineEntry[] }) {
  if (!timeline || timeline.length === 0) {
    return (
      <div className="rounded-xl bg-white p-8 shadow text-center">
        <span className="text-3xl">📭</span>
        <p className="mt-2 text-sm text-gray-500">No company lifecycle events yet.</p>
      </div>
    );
  }
  return (
    <div className="rounded-xl bg-gray-50 p-6 shadow">
      <h3 className="mb-6 text-sm font-semibold uppercase tracking-wider text-gray-500">Company Timeline</h3>
      <div>
        {timeline.map((entry, idx) => (
          <TimelineItem key={`${entry.type}-${entry.entityId}-${entry.timestamp}-${idx}`} entry={entry} isLast={idx === timeline.length - 1} />
        ))}
      </div>
    </div>
  );
